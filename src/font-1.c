 ...
 * Bit 7 of each byte is the leftmost pixel, which matches font8x8_basic. */
static void build_glyph_list(unsigned char ch) {
    ...
    for (int r = 0; r < 8; r++) {
        uint8_t row = (uint8_t)font8x8_basic[ch][r];
...
void Font_DrawDigit(int texId, int digit, int x, int y, float scale)
{
    ...
    int texH = Texture_GetHeight(texId);
    if (texH <= 0) texH = 256;
    int glV1 = texH - v2;
    int glV2 = texH - v1;

    Texture_DrawUV(texId, (float)x, (float)y, (float)w, (float)h,
                   (float)u1, (float)glV1, (float)u2, (float)glV2,
                   1.0f, 1.0f, 1.0f, 1.0f);
}
