...
{
    snprintf(out, n, "%s/PUMPY.INI", g_game.currentDirectory);
}

/* Lê o PUMPY.INI antigo (antes da imagem EEPROM). Se all == false, só o AudioOffset. */
static bool loadIni(bool all)
{
    char path[MAX_PATH];
